...
def test_load_targets_can_reprocess_existing_low_fee_rows() -> None:
    ...
    conn.execute(
        ...
        INSERT INTO instrument(instrument_id, isin, instrument_name, issuer_id, universe_mvp_flag, ucits_flag)
        VALUES
            (1, 'LU0000001001', 'Amundi Suspicious Low Fee UCITS ETF', 1, 1, 1),
            (2, 'LU0000001002', 'Amundi Normal Fee UCITS ETF', 1, 1, 1)
        """
    )
    conn.execute(
        """
    ...
    conn.executemany(
        """
        INSERT INTO cost_snapshot(instrument_id, asof_date, ongoing_charges, quality_flag, raw_json)
        VALUES (?, '2026-03-07', ?, 'amundi_factsheet_ok', '{}')
        """,
        [
            (1, 0.01),
            (2, 0.12),
    ...
    conn.executemany(
        ...
        INSERT INTO product_profile(
            ...
            hedged_flag,
            updated_at
        ) VALUES (?, ?, '2026-03-07', 'MSCI World', 'Equity', 'Luxembourg', 100000000.0, 'physical', 0, '2026-03-07T00:00:00Z')
        """,
        [
            (1, 0.01),
            (2, 0.12),
...
def test_main_commits_successful_rows_before_later_failure(
    ...
    monkeypatch.setattr(amundi_enrich, "load_targets", lambda conn, limit, venue, max_existing_fee=None: rows)
