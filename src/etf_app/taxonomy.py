...
def classify_instrument(
    *,
    ...
    original_text = instrument_name or ""
    text = _normalize_name(instrument_name)
    ...
    for commodity_name, key in (
        ...
        if gold_flag == 1:
            commodity_type = "gold"
            break
        if _has_any(text, COMPILED_PATTERNS[key]):
            commodity_type = commodity_name
            evidence.append(f"commodity:{commodity_name}")
            break

    cash_proxy_flag = 1 if _has_any(source_text, COMPILED_PATTERNS["cash"]) or _has_any(text, COMPILED_PATTERNS["cash"]) else 0
    cash_flag = cash_proxy_flag
    multi_flag = _has_any(text, COMPILED_PATTERNS["multi"])
    bond_flag = _has_any(text, COMPILED_PATTERNS["bond_core"]) or _has_any(original_text.upper(), COMPILED_PATTERNS["bond_special"])
    govt_bond_flag = 1 if _has_any(text, COMPILED_PATTERNS["bond_type_govt"]) or _has_any(original_text.upper(), COMPILED_PATTERNS["bond_special"]) else 0
    low, high = _parse_duration_bounds(source_text)
    ...
    if _has_any(text, COMPILED_PATTERNS["style_value"]):
        equity_style = "value"
        evidence.append("style:value")
    elif _has_any(text, COMPILED_PATTERNS["style_growth"]):
        equity_style = "growth"
        evidence.append("style:growth")

    geography_country, region_from_country = _match_country(text)
    ...
    elif _has_any(text, COMPILED_PATTERNS["region_global"]):
        geography_region = "global"
        geography_scope = "global"
        evidence.append("region:global")
    elif _has_any(text, COMPILED_PATTERNS["region_em"]):
        geography_region = "em"
        geography_scope = "regional"
        evidence.append("region:em")
    elif _has_any(text, COMPILED_PATTERNS["region_europe"]):
        geography_region = "europe"
        geography_scope = "regional"
        evidence.append("region:europe")
    elif _has_any(text, COMPILED_PATTERNS["region_asia"]):
        geography_region = "asia"
        geography_scope = "regional"
        evidence.append("region:asia")

    equity_hint = _has_any(text, COMPILED_PATTERNS["equity_hint"]) or bool(
        geography_region or geography_country or sector or factor or equity_size or equity_style or theme
    )
    if original_text.upper().startswith("JPM-") and " EQ " in f" {text} ":
        equity_hint = True
    ...
    if asset_class == "bond":
        if _has_any(text, COMPILED_PATTERNS["bond_type_linkers"]):
            bond_type = "linkers"
        elif govt_bond_flag == 1:
            bond_type = "govt"
        elif _has_any(text, COMPILED_PATTERNS["bond_type_aggregate"]):
            bond_type = "aggregate"
        elif _has_any(text, COMPILED_PATTERNS["bond_type_corp"]):
            bond_type = "corp"
        else:
            bond_type = "unknown"
    ...
    for target, patterns in COMPILED_HEDGED_TARGETS:
        if _has_any(text, patterns):
            hedged_flag = 1
            hedged_target = target
            evidence.append(f"hedged:{target}")
            break
    if hedged_flag == 0 and "HEDGED" in text:
        hedged_flag = 1
        evidence.append("hedged:explicit")

    domicile_country = _domicile_country(isin)
    if domicile_country:
        evidence.append(f"domicile:{domicile_country}")
    ...
    return TaxonomyResult(
        ...
        evidence_json=json.dumps({"rules": evidence, "normalized_name": text}, ensure_ascii=True),
